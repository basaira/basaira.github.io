...
async function setMotionBoundary(page,phase){
...
async function center(page,sel){return page.$eval(sel,e=>{const b=e.getBoundingClientRect();return{x:b.left+b.width*.5,y:b.top+b.height*.5}});}
...
async function openPair(browserS,browserT,originS,originT,{mobile=false,reduced=false}={}){
  const s=await browserS.newPage(),t=await browserT.newPage(),sm=await configurePage(s,originS,{mobile,reduced}),tm=await configurePage(t,originT,{mobile,reduced});
...
const tempRoot=fs.mkdtempSync(path.join(os.tmpdir(),'phase3b5c-')),sourceServer=await serverFor(source);
...
const environment={chromeVersion:await browserSource.version(),sourceProcessIndependent:true,syntheticProcessIndependent:true,viewports:VIEWPORT_WIDTHS,locales:LOCALES,forcedColorsApplicable:Boolean(staticData.forcedColorsUsed),intermediateAnimation:{tested:false,reason:'Mixed delayed CSS transitions/keyframes are characterized at deterministic production class boundaries (motion-prep and motion-entered settled) rather than unsynchronized wall-clock timestamps.'}};
const pairResults=[];
try{
  for(const pair of PAIRS){
    const variant=constructVariant(pair,tempRoot),synServer=await serverFor(variant.dir),records=[],unresolved=[];let resourceFailures=[],fineCaps=null,coarseCaps=null,reducedCaps=null,cssomBoundary=null,nativeAnchor={};
    try{
      ...
      await Promise.all([setMotionBoundary(s,'prep'),setMotionBoundary(t,'prep')]);
      ...
      recordCompare(records,'initial-pre-animation','motion',await snapshot(s),await snapshot(t));
      await Promise.all([setMotionBoundary(s,'entered'),setMotionBoundary(t,'entered')]);recordCompare(records,'initial-settled','motion',await snapshot(s),await snapshot(t));
      cssomBoundary={source:(await snapshot(s)).cssom,synthetic:(await snapshot(t)).cssom};

      for(const width of VIEWPORT_WIDTHS){
        await Promise.all([s.setViewport({width,height:width<=640?844:900,deviceScaleFactor:1,isMobile:false,hasTouch:false}),t.setViewport({width,height:width<=640?844:900,deviceScaleFactor:1,isMobile:false,hasTouch:false})]);
        for(const lang of LOCALES){await Promise.all([setLocale(s,lang),setLocale(t,lang)]);await settlePair(s,t);recordCompare(records,`responsive:${width}:${lang}`,'responsive',await snapshot(s),await snapshot(t));}
      }

      await Promise.all([s.setViewport({width:1440,height:900,deviceScaleFactor:1,isMobile:false,hasTouch:false}),t.setViewport({width:1440,height:900,deviceScaleFactor:1,isMobile:false,hasTouch:false}),setLocale(s,'en'),setLocale(t,'en')]);await settlePair(s,t);recordCompare(records,'fine-idle','interaction',await snapshot(s),await snapshot(t));
      await Promise.all([moveRatio(s,'#home .hero-primary-cta',.55,.45),moveRatio(t,'#home .hero-primary-cta',.55,.45)]);await settlePair(s,t);recordCompare(records,'fine-hover-primary','interaction',await snapshot(s),await snapshot(t));
      await Promise.all([moveRatio(s,'#home .hero-visual-stage',.72,.33),moveRatio(t,'#home .hero-visual-stage',.72,.33)]);await settlePair(s,t);recordCompare(records,'fine-stage-pointer-custom','interaction',await snapshot(s),await snapshot(t));
      await Promise.all([moveRatio(s,'#home .hero-primary-cta',.33,.64),moveRatio(t,'#home .hero-primary-cta',.33,.64)]);await settlePair(s,t);recordCompare(records,'fine-cta-pointer-custom','interaction',await snapshot(s),await snapshot(t));
      await Promise.all([s.mouse.down(),t.mouse.down()]);await settlePair(s,t);recordCompare(records,'fine-active-primary','interaction',await snapshot(s),await snapshot(t));
      await Promise.all([s.mouse.move(1,1),t.mouse.move(1,1),s.mouse.up(),t.mouse.up()]);await settlePair(s,t);recordCompare(records,'fine-acknowledged-after-press','interaction',await snapshot(s),await snapshot(t));
      await sleep(470);await settlePair(s,t);recordCompare(records,'fine-pointer-leave-reset','interaction',await snapshot(s),await snapshot(t));
      await Promise.all([focusPrimaryByKeyboard(s),focusPrimaryByKeyboard(t)]);await settlePair(s,t);recordCompare(records,'keyboard-focus-primary','interaction',await snapshot(s),await snapshot(t));
      await Promise.all([s.keyboard.press('Tab'),t.keyboard.press('Tab')]);await settlePair(s,t);recordCompare(records,'keyboard-focus-secondary','interaction',await snapshot(s),await snapshot(t));
      await Promise.all([setLocale(s,'ar'),setLocale(t,'ar')]);await Promise.all([moveRatio(s,'#home .hero-primary-cta',.5,.5),moveRatio(t,'#home .hero-primary-cta',.5,.5)]);await settlePair(s,t);recordCompare(records,'rtl-primary-hover','interaction',await snapshot(s),await snapshot(t));
      await Promise.all([setLocale(s,'en'),setLocale(t,'en')]);await Promise.all([s.evaluate(()=>document.querySelector('#home .hero-secondary-cta')?.click()),t.evaluate(()=>document.querySelector('#home .hero-secondary-cta')?.click())]);nativeAnchor={sourceHash:await s.evaluate(()=>location.hash),syntheticHash:await t.evaluate(()=>location.hash)};if(nativeAnchor.sourceHash!=='#quick-guide'||nativeAnchor.syntheticHash!=='#quick-guide')unresolved.push({type:'native-anchor-navigation',nativeAnchor});await Promise.all([s.evaluate(()=>window.scrollTo(0,0)),t.evaluate(()=>window.scrollTo(0,0))]);await settlePair(s,t);recordCompare(records,'native-anchor-secondary','interaction',await snapshot(s),await snapshot(t));
      ...
      const coarse=await openPair(browserSource,browserSynthetic,sourceServer.origin,synServer.origin,{mobile:true});coarseCaps={source:await capabilities(coarse.s),synthetic:await capabilities(coarse.t)};
      if(!coarseCaps.source.pointerCoarse||!coarseCaps.source.hoverNone||!coarseCaps.synthetic.pointerCoarse||!coarseCaps.synthetic.hoverNone)unresolved.push({type:'coarse-capability',coarseCaps});await Promise.all([setMotionBoundary(coarse.s,'entered'),setMotionBoundary(coarse.t,'entered')]);recordCompare(records,'coarse-idle','interaction',await snapshot(coarse.s),await snapshot(coarse.t));
      const c1=await center(coarse.s,'#home .hero-primary-cta'),c2=await center(coarse.t,'#home .hero-primary-cta'),cs=await coarse.s.target().createCDPSession(),ct=await coarse.t.target().createCDPSession();
      await Promise.all([cs.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x:c1.x,y:c1.y}]}),ct.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x:c2.x,y:c2.y}]})]);await settlePair(coarse.s,coarse.t);recordCompare(records,'coarse-touch-active','interaction',await snapshot(coarse.s),await snapshot(coarse.t));
      await Promise.all([cs.send('Input.dispatchTouchEvent',{type:'touchCancel',touchPoints:[]}),ct.send('Input.dispatchTouchEvent',{type:'touchCancel',touchPoints:[]})]);await sleep(470);await settlePair(coarse.s,coarse.t);recordCompare(records,'coarse-touch-reset','interaction',await snapshot(coarse.s),await snapshot(coarse.t));resourceFailures.push(...coarse.sm.failures,...coarse.tm.failures);if(coarse.sm.pageErrors.length||coarse.tm.pageErrors.length)unresolved.push({type:'coarse-page-errors',source:coarse.sm.pageErrors,synthetic:coarse.tm.pageErrors});await Promise.all([coarse.s.close(),coarse.t.close()]);

      const reduced=await openPair(browserSource,browserSynthetic,sourceServer.origin,synServer.origin,{reduced:true});reducedCaps={source:await capabilities(reduced.s),synthetic:await capabilities(reduced.t)};
      if(!reducedCaps.source.reduced||!reducedCaps.synthetic.reduced)unresolved.push({type:'reduced-capability',reducedCaps});await Promise.all([setMotionBoundary(reduced.s,'prep'),setMotionBoundary(reduced.t,'prep')]);recordCompare(records,'reduced-initial','motion',await snapshot(reduced.s),await snapshot(reduced.t));await Promise.all([setMotionBoundary(reduced.s,'entered'),setMotionBoundary(reduced.t,'entered')]);recordCompare(records,'reduced-settled','motion',await snapshot(reduced.s),await snapshot(reduced.t));resourceFailures.push(...reduced.sm.failures,...reduced.tm.failures);if(reduced.sm.pageErrors.length||reduced.tm.pageErrors.length)unresolved.push({type:'reduced-page-errors',source:reduced.sm.pageErrors,synthetic:reduced.tm.pageErrors});await Promise.all([reduced.s.close(),reduced.t.close()]);
    }catch(e){unresolved.push({type:'harness-exception',message:String(e?.stack||e)});}
    await new Promise(r=>synServer.server.close(r));
    const totals=aggregate(records),staticPair=staticData.pairs.find(x=>x.id===pair.id),runtimeFilenameDep=(staticPair?.runtimeFilenameHits||[]).length>0,targetCssomRefs=(staticData.cssomRefs||[]).filter(r=>r.text.includes(pair.first)||r.text.includes(pair.second)),productionCssomObserver=targetCssomRefs.length>0,resourceCount=resourceFailures.length;
    const unexpected=totals.computed+totals.geometry+totals.custom+totals.state+totals.accessibility+resourceCount+unresolved.length;
    let disposition='A — SAFE EXACT-CONCAT PAIR',ownershipFinding='Behaviorally equivalent; no target-specific runtime filename/CSSOM dependency or protected ownership reason identified.';
    ...
    else if(pair.id==='P3'){disposition='B — KEEP SEPARATE — COMPONENT OWNERSHIP';ownershipFinding='Boundary separates independently JS-coupled Hero interaction domains (Overdrive pointer engine vs Delight acknowledgement).';}
    ...
    pairResults.push({pair,...variant,staticPair,targetCssomRefs,environment:{fineCaps,coarseCaps,reducedCaps},records,totals,resourceFailures,resourceFailureCount:resourceCount,unresolved,unresolvedCount:unresolved.length,cssomBoundary,nativeAnchor,ownershipFinding,disposition,pass:unexpected===0});
...
}finally{await Promise.allSettled([browserSource.close(),browserSynthetic.close()]);await new Promise(r=>sourceServer.server.close(r));fs.rmSync(tempRoot,{recursive:true,force:true});}

const totalComparisons=pairResults.reduce((n,p)=>n+p.totals.comparisons,0),blockers=pairResults.filter(p=>!p.pass||p.disposition.startsWith('E'));
const recommendedSequence={direct:[{step:1,pair:'P1',survivor:'hero-typeset-v2.css',retire:'hero-layout-v2.css',expectedProductionCss:51,rationale:'Isolated safe pair; typography/layout are pure CSS structural layers with no runtime filename/CSSOM blocker.'},{step:2,pair:'P4',survivor:'hero-delight-v4.css',retire:'hero-animate-v4.css',expectedProductionCss:50,rationale:'Isolated safe pair; survivor keeps Delight CSS beside its JS companion while absorbing adjacent entrance choreography. P1 is disjoint, so P4 source bytes/order are unchanged by step 1.'}],conditional:[{pair:'P5',status:'isolated-safe-alternative',note:'P5 is safe in isolation, but after P4 the left boundary becomes the combined Delight+Animate survivor. Re-characterize that new boundary before any further merge with hero-polish-v4.css.'}],keepSeparate:['P2','P3','P6'],invariant:'track-buttons-v6.css remains terminal; hero-cta-overdrive-v1.css remains isolated; no multi-file transitive collapse inferred.'};
const output={schema:'phase-3b-5c-browser-v2',generatedAt:new Date().toISOString(),baseSha:process.env.BASE_SHA||null,environment,totalComparisons,pairs:pairResults,recommendedSequence,ready:blockers.length===0};

...
async function interactionPoint(a,b,selector,rx=.5,ry=.5){
  const read=page=>page.$eval(selector,e=>{const r=e.getBoundingClientRect(),cs=getComputedStyle(e);return{left:r.left,top:r.top,right:r.right,bottom:r.bottom,width:r.width,height:r.height,display:cs.display,visibility:cs.visibility,pointerEvents:cs.pointerEvents};});
  const[ra,rb]=await Promise.all([read(a),read(b)]),interactive=x=>x.display!=='none'&&x.visibility!=='hidden'&&x.pointerEvents!=='none'&&x.width>0&&x.height>0,ia=interactive(ra),ib=interactive(rb);
  if(ia!==ib)throw new Error(`interaction applicability mismatch for ${selector}: source=${JSON.stringify(ra)} synthetic=${JSON.stringify(rb)}`);
  if(!ia)return{applicable:false,selector,source:ra,synthetic:rb};
  const left=Math.max(ra.left,rb.left),right=Math.min(ra.right,rb.right),top=Math.max(ra.top,rb.top),bottom=Math.min(ra.bottom,rb.bottom);
  if(right<=left||bottom<=top)throw new Error(`no common pointer area for interactive ${selector}: source=${JSON.stringify(ra)} synthetic=${JSON.stringify(rb)}`);
  return{applicable:true,selector,x:left+(right-left)*rx,y:top+(bottom-top)*ry,source:ra,synthetic:rb};
}
async function exactCommonPoint(a,b,selector,rx=.5,ry=.5){const p=await interactionPoint(a,b,selector,rx,ry);if(!p.applicable)throw new Error(`expected interactive target but ${selector} is non-interactive on both sides`);return{x:p.x,y:p.y};}
...
async function measurePair(a,b,monA,monB,mode,records,label,category,ctx={}){
  ...
  const[sa,sb]=await Promise.all([snapshot(a,{mode,label,locale:lang,viewport:{width,height},motionPhase,category},ra),snapshot(b,{mode,label,locale:lang,viewport:{width,height},motionPhase,category},rb)]),r=recordCompare(records,label,category,sa,sb,{readinessProblems,...ctx.recordMeta});if(readinessProblems.A.length||readinessProblems.B.length)r.readinessBlocker=true;return r;
}
...
async function run93Matrix(a,b,monA,monB,mode,records){
  ...
  const stagePoint=await interactionPoint(a,b,'#home .hero-visual-stage',.72,.33);if(stagePoint.applicable){await Promise.all([a.mouse.move(stagePoint.x,stagePoint.y),b.mouse.move(stagePoint.x,stagePoint.y)]);await Promise.all([settlePointerRaf(a),settlePointerRaf(b)]);}await Promise.all([normalizeAnimations(a,1),normalizeAnimations(b,1)]);await measurePair(a,b,monA,monB,mode,records,'fine-stage-pointer-custom','interaction',{locale:'en',viewport:{width:1440,height:900},motionPhase:'entered',recordMeta:{interactionApplicable:stagePoint.applicable,interactionTarget:stagePoint}});
